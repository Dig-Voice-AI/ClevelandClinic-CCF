...
public class DBEMPIDOBVerify extends DecisionElementBase implements ElementInterface {
	...
	public static final String REQ_PARAM_EMPI_DOB		= "empi_Dob";
	...
	throws ElementException {
		...
		try {
			...
			String empiDob = config.getSettingValue(REQ_PARAM_EMPI_DOB, data);
			//String empiDobValue=empiDob.substring(0,2)+"/"+empiDob.substring(2,4)+"/"+empiDob.substring(4,8);
			
			EMPIDBDa da = Util.createObject(componentName, data, true);
			EMPIResponse cr = new EMPIResponse();
				
			cr = da.getEmpiDobResponse(empiValue, empiDob);
			...
			if(cr.empiResponse.equals("0"))
			{
				...
				data.addToLog("INFO", "No EMPI/DOB found");				
			}
			else
			{
				exitState = EXIT_STATE_FOUND;
				data.addToLog("INFO", "Found " + cr.empiResponse + " EMPI : " + empiValue);// + " DOB : " + empiDobValue);		
			}
